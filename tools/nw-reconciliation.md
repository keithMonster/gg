...
|---|---|---|---|
| **轨1 自动闭环** | 回写 status=done + resolution + `resolution_origin` + `auto_done_at`；thread 类 append 时间线；canon-bugs 类 append 症状反查；skill 类合并经验段 | ① 落点 ∈ 白名单 {`threads/*.md` 时间线 append / `canon-bugs.md` 症状反查 append / `~/.agents/skills/*/SKILL.md`(author:monster) 经验段 append / `harness/{lib,analysis}/**`} ② 可逆（纯 append/增量，git 一条 revert，不含删除/重写/status 翻转）③ **物理核验过**（见下「物理核验两查」+ canon-bugs schema-shape） | 任一不满足 → L4 |
...
- **canon-bugs schema-shape**：目标是 `canon-bugs.md` 时，核 a/核 b 之外必须检查待 append 条目格式，至少匹配 `- YYYY-MM-DD [症状: A / B / ...] → 根因 ... → 修 ... → 延伸 ...`，且 `症状:` 中至少 2 个 `/` 分隔的自然变形；不满足即 L4 上浮。schema 可机检，语义正确性不可机检，二者不要混。
...
- `canon.md` 永久人工 apply。它是带理由链的规矩文件，append 一条规矩就是价值判断；不要因 `canon-bugs.md` 进入白名单而顺手放宽 `canon.md`
...
2. **分轨**：按 type / 落点 / 动作性质把 pending/blocked/deferred 条目分轨：
   - **轨1 桶**：落点命中白名单（thread 时间线 append / canon-bugs 症状反查 append / author:monster skill 经验段 / harness lib·analysis）+ 纯 append/增量
   - **轨2 桶**：含删除/移动等不可逆动作
...
4. **执行**：
   - **轨1**：
     ...
     - canon-bugs append 类 → Read canon-bugs.md → 核 a/核 b + schema-shape → Edit append 症状反查条目 → 回写 status=done + `resolution_origin: "auto-track1-canonbugs-append"` + `auto_done_at`
...
**版本**：v0.3.1（2026-06-29 轨1 白名单纳 `canon-bugs.md` append + schema-shape 检查；`canon.md` 明确永久人工）/ v0.3.0（2026-06-09 准入判据 confidence→动作性质+物理核验，对齐 monster 5-18 机械类自动落地契约 + 拆轨1/2/3a/3b + 加 thread-append 轨1 档——补 gg 仓欠债 + 修"自评当准入"根病，实证 auto-from-draft 一个月 0 触发）/ v0.2.0（2026-05-06 取消"保留不动桶" + L1 接受高置信 draft）/ v0.1.0 首建 2026-04-23
