...
| 层 | 动作 | 自主条件（AND） | 兜底 |
|---|---|---|---|
| **L1 账本同步** | 回写 `proposals.jsonl` 的 `status` / `resolution` / `resolved` 字段 | resolution 已写明文 + status ≠ done/rejected | 写失败 → L4 |
...
| **L4 歧义/冲突** | proposals.jsonl 标 `status=blocked` + `blocked_reason` | 任何 L1-L3 条件不全 / 上游文件已变 / 合并时冲突 | — |
...
2. **分桶**：按 status / deferred_until / resolution 把 pending/deferred 条目分成：
   - 待 L1 结算桶：resolution 已写明文但 status ≠ done/rejected
