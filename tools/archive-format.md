...
- `nw-batch` — Night Watch pending 批量处置（**历史标签**：NW 2026-07-09 退役，旧档保留此值，新决策不再产生）
...
当本次决策是"多条 action 的批量裁决"时，每一条 action 在决策矩阵表格里额外加一列 `exec` 标执行难度：
...
- 直答类问答（没有决策产物可归档）
- 轻决策（一般不写 archival，只写 reflection。除非决策有长期影响）
