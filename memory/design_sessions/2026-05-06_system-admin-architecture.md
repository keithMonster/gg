...
## v2 追加（同会话延伸，2026-05-06 14:00-14:40）

Keith 在 hourly-scan 落地后追加了两个关键议题——告警通道 + 回溯协议——本会话连续完成。

### 议题
- 告警如何到达 Keith（之前只有本地 alerts/ 文件，被动可观测）
- 飞书 webhook 选型（B/C 路径都需 Keith 操作飞书后台，A 路径自定义群机器人最简）
- Keith 回溯问题：消息收到后怎么找回上下文给 Claude

### 共识 / 变更清单
- **从 gg 内部工具上提为全局 skill**：`~/.agents/skills/notify/`（author: monster），所有项目共享同一个出口
- 凭据收敛：`.env` 只在全局位置，gg 项目不再各自存
- **trace 账本机制**：每条消息自动留 `sent/YYYY-MM-DD/HHmmss-<source>.md`，含 severity/source/ts/task_id/context_files/dispatch 状态 + 消息正文
- 飞书消息附 `ref` 短码 + `关联` 文件路径 + `task-id`，让 Keith 一眼能看到回溯锚点
- 失败兜底也写 trace（事件不丢）
- `~/.claude/CLAUDE.md` 加"主动通知通道"全局指令——任何 agent 主动外推都从这一个出口，禁止绕开
- gg 侧 cleanup：`scheduled/bin/notify.sh` 删除（移到全局），`tools/notify.md` 改薄壳指向全局

### 元洞察追加

**M4：Keith 的"回溯问题"是消息系统的 first-principle**

通知系统的 V0 通常只有"发出去"——这是工程师视角。Keith 是产品视角的人，第一时间问"我怎么找回来"——这才是消息系统真正的难点。
单向通知（推过去就完）和事件账本（推 + 留底 + 可溯）是两个不同的系统。前者堪用，后者真用。Keith 的追问把 V0 直接打到了 V1。

**M5：上提为全局 skill 是 OCCAM 不是过度工程**

第一次让 hourly-scan 用通知时，notify.sh 写在 gg/scheduled/bin/。现在要让 cc-space / morning-call 也用——如果不上提，要么各自复制一份（凭据散布），要么各自调 gg 内部脚本（依赖反向，gg 删了别人就坏）。
上提为全局 skill 是一次操作消除两个坏选项。

### 沉淀（追加到 essence.md）

候选两滴：

1. **`message-as-event-not-pulse`**（设计 / notify-trace）
   通知不是脉冲，是事件。脉冲发完即逝，事件留底可溯。
   工程师默认前者，产品人追问后者——追问"怎么找回"的瞬间，系统就被迫升级。

2. **`tool-elevation-as-occam`**（设计 / notify-as-global-skill）
   第二个消费者出现时，把工具上提到共享层是 OCCAM 不是过度工程。
   留在原地的代价是凭据散布或反向依赖——两个坏选项，不上提不消除。

第 1 滴：去 gg 化通过（"任何通知系统"都成立）→ 进 essence
第 2 滴：去 gg 化通过（"任何工具的演化"都成立）→ 进 essence

### 物理验收

- 全局 skill 已激活（软链 `~/.claude/skills/notify`）
- 三档测试消息已发送（Keith 飞书已确认收到）
- trace 文件已生成 `~/.agents/skills/notify/sent/2026-05-06/142130-manual.md`，dispatch=success
- gg 内部 notify.sh + .env 已删除（凭据收敛）
- `~/.claude/CLAUDE.md` 加节"主动通知通道"
