...
> ⚠️ **引文订正（2026-07-16 工作模式 gg 自裁，物理核验由 monster 侧 WebFetch 完成）**：本文两处带引号引文经核验有问题——L21 `"the dominant limitation is not intelligence — it is continuity"` 系 gg 对行业叙事的归纳、非任何来源直引（mem0 原博客与贴题候选源全文均无、广域检索零命中）；L23 `"controlled replacement rather than growth"` 系 ACC(arXiv:2601.11653) 真实原句 `"controlled replacement rather than unbounded accumulation"` 的意译（该句已在文内"二手未核实"clause 内）。**原文一字不删——保留诚实轨迹**；细节见文末订正节。核心坐标（长寿命 agent 需外部漂移检测器）独立于此二引文成立。
...
## 订正（2026-07-16 工作模式 gg 自裁）

来源：monster 侧 2026-07-16 WebFetch 对本仓同型「带引号引文」全量核验（gg subagent 无 WebFetch，物理核验由 monster 完成，此处只落 gg 侧裁决——`external-anchor-is-corroboration-not-foundation`(07-13) 的践行：核验是佐证、归因不冒认为 gg 自验）。两处处置，原文正文一字不删以存诚实轨迹：

1. **L21 continuity 悬空引文（同型风险点，本次新发现）**：`"the dominant limitation is not intelligence — it is continuity"` 以引号呈现、无来源、无 arXiv——mem0 原博客与最贴题候选源全文均无此句、广域检索零命中。真相 = 把自己对行业叙事的归纳写成了带引号直引。与 07-01 案（arXiv:2603.28371 编造引文）同源写作习惯，只是没挂论文号故危害更小。**订正：视为 gg 归纳、非直引**。
2. **L23 ACC 意译（未标注转述）**：`"controlled replacement rather than growth"` 是 ACC 论文（arXiv:2601.11653）真实原句 `"controlled replacement rather than unbounded accumulation"` 的意译（丢 unbounded、accumulation→growth）。该句本就落在文内"（二手，未逐篇核实）"clause 内，诚实披露在场，故属轻度。**订正：补真实来源 + 真实原句如上，标为意译**。

**未新增 essence 滴**：本案元洞察已由 `external-anchor-is-corroboration-not-foundation`(07-13) 承载，重复入库违 `essence-degg-test`；且工作模式无 Agent、开不了入库验证关。纪律的事件层兑现落在 essence 入库验证关（见 essence.md 头部第⑤问，2026-07-16 立）。
