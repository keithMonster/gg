...
| `memory/essence.md`（当前卷） | > 500 滴 或启动成本可感 | `grep -c '^## 20'` | 分卷已执行 2026-08-01（#1–#186 → `essence/2026-H1.md`，commit ef50fce）；当前卷从 0 重计，再越线按 KERNEL §3 半年卷制续分。原锚记录（KERNEL §3 / essence.md 头部），此前越 1000 行不单独触发 |
...
```bash
# 视图必须含 essence 全部 slug（当前卷 + 归档卷，2026-08-01 分卷起双路径）——漏一个 = 该滴 grep 不到 = 从启动记忆静默消失
python3 -c "
import re, glob
ess=set()
