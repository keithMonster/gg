...
| id | 首报 | 内容 | 状态 / 出口条件 |
|---|---|---|---|
| P-0720-nonfire-recur | 07-20 | **07-15 / 07-16 双夜 auto_gg 全暗**（无日志、无 commit；gg-explore 槽同期同暗——explore 档 07-15 后直接跳 07-18）。物理核：`git log 07-14..07-18` 该两夜零 gg 产出；07-16 07:19 的 plist 归档 commit(c699a0a) **不是成因**（客户端 routine 自 06-12 起即唯一执行者，plist 06-12 已 bootout）。**P-0707-nonfire 于 07-13 以"一次性未复发"结案属过早**——两天后即复发且连发两夜 | §5「连续 2 次同类问题」触发：已推 agenda `[RECURRING]` 交 Keith 查客户端调度根因（定时任务配置在 gg 权力边界外，gg 不自查不自修）。**出口**：Keith 定位根因并处置 / 或此后连续 14 个日历日无 **non-fire**。**07-24 加注**：07-23 的缺日志**不是 non-fire**（该夜 fired 且 committed 8a72baf，属独立子模式 collapse-before-log → 见 P-0702-missing-log），故本条 non-fire 计数 07-17→07-24 未断（07-17~22 有日志、23 fired、24 本夜）。**07-28 状态变化**：Keith 当日把三条 AI 任务从客户端迁回 launchd（9170ffe + 6c11783）——原根因载体（客户端调度）整体退役，出口第一支「处置」以调度器更换形态发生；本夜为 launchd 触发首夜、实际 fired（本日志即产物）。计数 07-17→07-28 连续 12 日无 non-fire；观察对象自本夜起换为 launchd 调度，若至 07-30 无 non-fire 即满 14 日按出口第二支结案 |
| P-0702-missing-log | 07-02（重开 07-24） | **auto_gg commit 无对应日志**。首例 06-13（commit ad2cd74 产 essence+agenda+tracks 却无 memory/auto_gg/2026-06-13.md）；**07-23 复发**（commit 8a72baf 只更 substrate 传感器节、无日志，`git log --all -- .../2026-07-23.md` 证从未 commit）。同形态第 2 次 → 按原结案条"复发则重开并按 §5 [RECURRING] 处理"重开。**顺带证伪暗夜哨前提**：哨假设"缺日志⟹non-fire"，07-23 反例（fired+committed 仍无日志）= 独立子模式，与 07-15/16 non-fire 两回事 | §5 触发：已推 agenda `[RECURRING]` 交 Keith 查根因（日志物理缺失可核；"从未创建 vs 创建后丢失"会话内不可判，疑似 session 生命周期——commit 后结束于写日志前，属 gg 权力边界外）。**07-23 实况仅存于 8a72baf commit message，日志永久缺失不补造**（`铁律2` 禁伪造）。**出口**：Keith 定位根因 / 或此后连续 14 个日历日无缺日志夜。**07-28 加注**：疑似根因载体（客户端 routine session 生命周期）随 07-28 迁回 launchd（9170ffe）退役，缺日志计数 07-24→07-28 连续 5 日无缺；出口条件不变，观察对象换为 launchd 会话 |
