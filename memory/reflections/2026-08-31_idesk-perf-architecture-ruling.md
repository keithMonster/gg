...
**追问轮补充裁定（同会话第二轮，父会话两点质询后）**：
- ① rechunk 28563ms 属已死实现（08-28 批量化前），采纳——原建议③降档：结构论证保留（重切成本随文档长度无上界、批量化只缩常数不缩界），但失去活实证，退到「仪表量过再排」，落于 AI 并发闸之后（后者仍有 08-28 12:14 独立实证）。
- ② 鉴权出站熔断/降级判**并列放行，且与仪表是同一次改动的两面**。裁决判据（新）：「没定根因不许买」禁的是**只在特定根因下才有收益**的解法（Redis 属此）；对全部候选根因收益恒正的边界修复不受此禁（每请求独立等满 3 秒无共享知识 = weilu 自身策略缺陷，与上游为何慢无关）。执行拆半：免校准半边（传输层失败短 TTL 负缓存 + fail-fast）随仪表同批；需校准半边（熔断阈值 / stale 扩展时长）等几天上游延迟基线再定，且 stale 扩展仅限曾验证通过的身份、时长有界（撤销余命是被定价过的安全参数）。硬约束：熔断必须自带状态迁移与上游耗时账本——不带观测的熔断把症状治哑。
