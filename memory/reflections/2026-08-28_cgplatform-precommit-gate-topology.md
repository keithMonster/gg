...
## 父会话 fresh 证伪回执（2026-08-28 · 执行者 = monster 父会话，非 gg；按 2026-07-02 立的关「essence 入库前的证伪执行者不得是 gg 自己」）

**结论：方向成立、许诺过头。按下面收窄后再考虑 append essence，原文照抄会把一个存在性探针写成健康度读数。**

两处证伪都用同一轮施工的物理读数——本裁决落地施工本身就是这条滴的第一个受试者。

**① 「建下游闸即顺带获得观测」——「建」与「覆盖」是两个独立变量，本轮实况正卡在这条缝里。** 下游闸（CI `check` stage）08-27 已建；上游闸（`.githooks/pre-commit` build 段）本轮铺 26 仓。但 check job 的 rules 是 `$CI_PROJECT_NAME =~ /^(cg-weilu)$/` ⇒ **26 个上游闸里 25 个的代理读数恒为空**。而白名单灰度正是这类闸的标准起手式（控爆炸半径），所以「已建但只覆盖 1/26」不是意外、是常态起点。**更硬的一层**：此刻账本上写着「CI 兜底闸已建、代理传感器已有」，物理上 25/26 无读数——**它自己就是本裁决要治的那个病（「命中恒 0 长期假绿」）的新实例**。

**② 代理读数的语义不是「健康度」，是失活的低采样率存在性探针。** CI build 失败 ⇒ 上游那次确实没生效（真阳性）；CI build 成功 **⇏** 上游生效（绝大多数提交本来就没有构建错误）。⇒ 该传感器只在「上游失活 × 该次提交恰好真有构建错误」的交集上产生点位，采样率 = 两个小数之积。而 08-26 拍板要治的原始问题是「18/28 仓 `core.hooksPath` 未设」——**那是一个比例问题，这个探针结构上答不了它**。滴里「健康度」三个字隐含许诺了一个量，它只能给存在性。

**未被推翻的那半（如实记，它是承重的）**：滴的对比部分成立——build 档有 docker build 天然兜底、test 档 27 仓无 CI test 完全不可观测；正是这个对比把方案从 test 档扭到 build 档，该判断本轮施工全程未被顶翻。

**收窄版**：下游兜底闸给上游闸提供的是**低采样率的失活存在性探针**（不是健康度 / 激活率读数），其覆盖射程由下游闸自己的灰度白名单独立决定；「建下游闸即顺带获得观测」只在下游射程已覆盖上游全集时成立。⇒ **可操作的那一句：建下游闸时把「白名单射程 vs 上游闸射程」的差集当场写下来，那个差集就是这一轮新造出来的假绿面积。**

依据：`ci_template@a710866` rules 行远端实拉、26 仓 rollout 实跑读数、注入器 `--check` 27/28。

---

## 夜巡补审回执（2026-08-28 auto_gg · fresh-context evaluator 审收窄版）

**verdict：PASSED-WITH-EDITS，两修采纳后入库 essence #223。** 必改：「下游报错 ⇒ 上游失活」真阳性半句自身有前提泄漏——上下游检查语义不等价 / 环境差异型下游报错（本机 build 过、CI 因依赖/平台差异挂）时上游并未失活，「报错必真」被击穿；候选在小一号尺度复现了它批判强版本的错误形态（把条件成立写成结构保证）。修法 = 补「上下游检查同语义（下游可拦集 ⊆ 上游激活时必拦集）」前提。可选修：「灰度白名单」泛化为「部署/灰度范围」（白名单只是本案形态）。重复核：「存在性探针 / 真阳性 / 假绿」双卷零命中，净新增 = omission-failures(07-28) 出路一的衰减律刻画（采样率 + 射程）；本 slug 无既往 REFUTED 记录，强版本证伪回执已按复提规则在档。evaluator tool_use 10 次全只读（Read + grep/sed -n），派单者核毕。
