...
- **候选滴（已补审入库）**：`recurrence-defense-pays-on-second-occurrence` — 防复发的税按第二次付费：第一次修复只留免费可检索痕迹（commit/账本），复发被第二次现场证实才升级为事故与结构消除；全称预防在修完时点写卡 = 把税付给 100% 的修复去防少数复发，是 `ghost-rules` 在防复发工程域的推广。物理证据：本轮零实证复发案例 + 63 仓 22/41 测试基建分布 + cg-platform-template 增量自带骨架。相关既有滴：`ghost-rules` / `failure-response-is-priced-by-expected-reliability` / `mutation-self-records-consultation-must-buy-its-trace`。
  **verdict（2026-08-27 auto_gg 夜巡 fresh 证伪审，PASSED-WITH-EDITS 四修采纳，入库 essence #221）**：最强反驳 = 「第二次现场证实」依赖检索动作认出复发，而候选自引的 #220 负面半边恰判「读观测不建即无」；主证据「零实证复发案例」是识别通道未建时的零读数，按 `signal-weak-vs-channel-dead`(05-19) 无定价资格——部分挡回：核心句承重在算术结构非经验断言，但须把通道条件写进前提栏才算挡回。四修：① 前提栏域切分（限复发成本有界且连续可观测域；不可逆/灾变域第二次即不可接受——Keith 原话「不要出现第二次」字面拒绝本帧一事落进前提）+ 通道条件（识别检索须真实发生，否则本滴退化为纯粹不付税）；② 谱系补最近亲 `premature-abstraction-tripwire`（净新增 = 零边际成本哨 + 复发即事故升级时点，防 07-14 暗重复先例）；③ 「ghost-rules 推广」收窄为「算术化落点」；④ 证据全标宣称级、承重显式挂结构论证。潜在冲突 `repair-caps-at-baseline`（不可逆域反向）由前提①切域化解。evaluator 输入：候选全文 + 证据清单四条（三条宣称级 + 一条缺席型）；自 grep 双卷含谱系注层 + agenda + candidate 标记全扫；tool_use 14 次全只读，派单者已核，无写操作。
