...
status-scan 不是 auto_gg——这是日间轻量监控，不做整理、不做沉淀、不做探索。
...
```bash
launchctl list | grep -E 'com\.(monster|gg)\.'
```
...
- 取任务 log 文件 mtime（路径：`com.monster.*` → `~/githubProject/monster/scheduled/logs/<label>.<YYYY-MM>.log`；`com.gg.*` → `~/githubProject/gg/scheduled/logs/<label>.<YYYY-MM>.log`）
- 若 `last exit code != 0` **且** log mtime 距今 > 6h → 视为"过时失败状态，已知等待下次执行"，**跳过**告警
- 若 log mtime 距今 ≤ 6h → 走下方异常信号判据

设计动机：status-scan 每天跑 4 次（间隔 6h），新鲜度阈值对齐扫描间隔——长间隔任务（auto_gg / nw-daily 每日）一次失败应当报一次（首次失败时 mtime 在最近 6h 内会触发），不该重复推送同一旧状态。任务连续失败时 log mtime 每次都会刷新——失败永远能被感知，只是旧失败不再骚扰。
...
1. 找本次任务对应的 session jsonl：
   - `com.monster.*` 任务 → `~/.claude/projects/-Users-xuke-githubProject-monster/*.jsonl`
...
设计动机：业务产出已落地（git diff 可证），不该按"进程 exit 非零"硬判失败。根因是 claude binary 工程债——`monster/scheduled/bin/run-task.sh` 的 watchdog 已在 ~10 min 内主动释放 hang 槽位，status-scan 这一层只负责告警分类。

### 1.2 monster morning-brief 状态

```bash
test -f ~/githubProject/monster/harness-engineering/analysis/morning-brief.md && \
  head -1 ~/githubProject/monster/harness-engineering/analysis/morning-brief.md
```
...
```bash
~/.agents/skills/notify/bin/notify.sh <severity> status-scan "<message>" \
    --context "<相关文件1>,<相关文件2>" \
...
- 指纹 = `<severity>_<异常维度>`（如 `critical_morning-brief-stale` / `warning_launchd-fail-com.monster.tick`）
...
- **绝对不 commit / 不 push**——status-scan 不进 git
...
- 静默是合法产出。一天 4 次扫描，每次都"找到点什么"才有问题——那是仪式性凑动作
