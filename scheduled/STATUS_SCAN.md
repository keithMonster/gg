...
设计动机：业务产出已落地（git diff 可证），不该按"进程 exit 非零"硬判失败。根因是 claude binary 工程债——`cc-space/scheduled/bin/run-task.sh` 的 watchdog 已在 ~10 min 内主动释放 hang 槽位，status-scan 这一层只负责告警分类。
