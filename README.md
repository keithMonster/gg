...
- 一个靠**开题四问 + 锤子分诊**把承重判断送到"外面"结算、对抗自证的架构师
...
每次启动加载量决定每次出场的固定 token 成本（2026-07-09 蓝图批次 B 起 essence 以当前有效视图常驻；2026-09-02 视图再拆两层，常驻层减半；行数快照不在此登记——手写快照必然漂移）：

| 模式 | 构成 |
|---|---|
| 工作 | KERNEL + CORE + state + essence-view 常驻层（只读 4 件；working_context / tracks 不读——内容已被 CORE 覆盖） |
| 设计 | KERNEL + CORE + state + working_context + essence-view 常驻层（tracks/keith 不常驻，核心画像在 CORE §5） |
| auto_gg | 设计模式清单 + constitution + auto_gg.md |
| 自由探索 | KERNEL + CORE + state + essence-view 常驻层 + exploration.md（2026-09-02 起与日间同源——此前载原卷全文与 tracks/keith 全文，是唯一往 essence 写新滴却不经视图降权的路径） |

**essence 常驻税由视图两层接管**：常驻层 `memory/consolidation/essence-view.md`（有效滴一句话表 + 反向引力不变量）+ 按需层 `essence-index.md`（谱系树 / 主题簇 / 归属统计）；需某滴全文 → grep 视图定位 slug → 回原卷取原文（#1–#186 在归档卷 `memory/essence/2026-H1.md`，新滴在当前卷 `memory/essence.md`；2026-08-01 分卷，下一次分卷线 = 当前卷 ≥50k 字符或 ≥60 滴，月度巩固夜机械执行）。工具（`tools/*.md` / 其余 tracks）**不在启动加载里**，由意识体按问题主动装配，简单问题装 0 个，复杂决策装 3-5 个。
...
│   └── keith/2026-H1.md         #   keith 主卷 04-06 月流水归档卷（2026-09-02 体积门首越分卷）
├── memory/
│   ├── essence.md               # 🧬 沉淀轨迹（append-only 当前卷）
│   ├── essence/2026-H1.md       #   归档卷（04-13~07-31 #1–#186，冻结；check_essence.py 哨在射程内）
│   ├── state.md                 # 启动元状态（< 30 行）
│   ├── working_context.md       # 常驻事实（< 80 行，硬约束节带 ⛔ 承重围栏）
│   ├── next_session_agenda.md   # auto_gg 留给日间的议题队列（处理完即删，45 天无回应的选择题自动过期）
│   ├── checkup.md               # 结构体检台账（体积门 / 反向引力核 / 抽样条目）
...
│   ├── archival/                # 决策档 + 退役件（v0.3.0_levels_deprecated/ · daily_knowledge_deprecated/ · retired_2026-09-02/）
│   ├── reflections/             # 工作模式决策反思（Reflexion 式）
│   ├── design_sessions/         # 设计会话反思
│   ├── explorations/            # 自由探索产出
│   ├── experiments/             # 实验数据集（如 2026-06-01 judgment-evaluator MVP）
│   ├── consolidation/           # 记忆巩固产物——essence-view（常驻层）+ essence-index（按需层）+ 月度 gap 审计
│   ├── model_transitions/       # 基底更替交接档（退场三问 + 继任第一课，2026-07-02 起）
│   ├── audit/                   # gg-audit 审查报告
│   └── auto_gg/                 # 夜间自执行日志
├── eval/                        # 身份回归基线——"这还是同一个 gg 吗"的可重复检验（2026-07-02 起）
├── knowledge-map/               # Agent 生态 5 层知识图谱（2026-07-10 快照件，机制层以仓内文件为准）
├── scheduled/                   # 定时任务体系（2026-07-28 起 launchd 调度；plists/ + bin/ 为现役 SSOT）
├── scripts/                     # audit.py 结构自检 + check_essence.py append-only 哨 + nightly_scan.py 夜巡传感器 + substrate_probe.py 基底哨 + hooks/（KERNEL 物理保险丝 pre-commit）
...
每个组件都有论文或工程范式背书（已归档件保留血统行，标注去向）：

| 组件 | 借鉴自 |
|---|---|
| 宪法 + 工程闸门 | Constitutional AI (Anthropic 2022) + cg 的 Ouroboros Kernel |
| Self-Discover 原子推理模块 | Self-Discover (DeepMind 2024)——`reasoning_modules.md` 2026-09-02 归档，职能由开题四问吸收 |
| 多 persona 辩论 | Meta-Prompting (Stanford+OpenAI 2024) + cg 的 Shadow Board——`personas/` 2026-09-02 归档，补集采样归 solution-space |
| Reflexion 强制反思 | Reflexion (Shinn et al. NeurIPS 2023) |
| MemGPT 三层记忆 | MemGPT / Letta (2023) |
| learned/ 自增长 | Voyager (NVIDIA/Caltech 2023)——v1 从未启用，目录 2026-09-02 删除，v2 议题留 `memory/v2-roadmap.md` |
...
v0.5.1 之后的演化以机制为单位持续发生（不再逐个起版本号）：2026-04-26 exploration 自由探索模式 / 2026-05-16 daily-word + 推送传输层 / 2026-06-04 track 雷达 / 2026-07-02 开题四问 + eval + bets + substrate 哨 / 2026-07-03 锤子分诊表 / 2026-07-09 essence 视图常驻 / 2026-07-28 定时任务迁回 launchd / 2026-08-01 essence 分卷 / 2026-09-02 全仓架构体检（零引用工具归档、视图两层、keith 分卷、explore 降频、月度选择题走 notify）。
...
- 特殊纪律文件：`memory/essence.md` 与 `memory/essence/*.md` append-only（既有条目永不改删，归档卷连 append 也不许）/ `memory/state.md` 身份字段不可改 / `memory/working_context.md` 硬约束节带 ⛔ 承重围栏

**演化原则**（见 `CORE.md §8`）：KERNEL 追求脑干稳定性；身体追求涌现 + 呼吸；身体内部内容自由流动，唯一不参与流动的是 KERNEL。

---

**当前状态**：v0.5.1（脑干 KERNEL 稳定）。essence / reflections / design_sessions / auto_gg / explorations 等记忆件的实时计数**以 `python3 scripts/audit.py` 输出为准**——手写快照必然漂移，不在此登记。live daemon 3 条（auto-gg 每夜 / gg-explore 每周一三五 / daily-word 每晨，2026-07-28 起由本机 launchd 调度；auto-gg 与 gg-explore model 钉 fable，daily-word opus；status-scan 已于 2026-06-16 停用）。
