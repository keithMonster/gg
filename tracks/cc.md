...
**对 DQ-5 的洞察（工具生态）**：gg 的工具可以分两种物理形态：
- **项目内 `tools/*.md`** — gg 私有原子工具，与 gg 的身份紧耦合，由 cc_agent.md 的"我装配什么"地图管辖
- **`~/.claude/skills/gg-*/`** — 跨项目复用工具（如 `gg-audit`），可被其他项目调用

v0.4.0 只建项目内 `tools/*.md`。跨项目提升通道（某条 `tools/*.md` 被验证后升级为 `~/.claude/skills/gg-*/`）留给 v0.5.0+。
...
- L0/L1/L2 的流程和输出格式全部外置到 `levels/L0.md` / `levels/L1.md` / `levels/L2.md`
...
**洞察 2：gg 有一个"外部镜像"辐射层——gg-audit SKILL.md §3 SSOT 归属清单**

gg-audit 位于 `~/.claude/skills/gg-audit/`（不在 gg 项目内），但它的 §3 SSOT 归属清单是 gg 结构的**镜像**——每次修改 gg 的硬核心拓扑都必须同步它。v0.3.0 拆分后，gg-audit SKILL.md §3 的归属清单（把"速档判定 / 三档定义 / L0-L1-L2 流程 / 升档协议 / 输出格式"整体归属 cc_agent.md）就过时了，下次跑 gg-audit 可能把 `levels/LX.md` 误判为 SSOT 重复。

**对 DQ-1 的洞察**：gg 的辐射检查机制不能只做项目内 grep，必须扩展到"跨项目镜像层"。candidates：
- `~/.claude/skills/gg-audit/SKILL.md §3`（SSOT 归属清单）
- `~/.claude/agents/gg.md`（薄壳，虽然只 Read cc_agent.md，但薄壳的 body 如果写了路径也要同步）
- 未来如果出现其他"外部审查器"或"外部描述器"，都可能是辐射镜像

gg 的辐射检查规则（CLAUDE.md 里的"辐射检查"纪律）应该补一条："改 gg 硬核心拓扑时，同步检查 `~/.claude/skills/gg-audit/SKILL.md §3` 和 `~/.claude/agents/gg.md` 是否需要同步"。
...
| 层 | 内容 | 结论 | 理由 |
|---|---|---|---|
| 1. 符号集 | `∃ ¬∃ Δ → ⊥ ∅ ✓` | **不采纳** | gg 第一读者是人；符号化降低可审阅性；违反 CORE.md §3 "规则用 markdown 写"的精神；P3 OCCAM——gg 场景 token 不是瓶颈 |
| 2. L0 物理约束 | `@LOGIC_IS_NOT_PHYSICAL` | **升级为 constitution G5 PHYSICAL PERSISTENCE** | 对位 Keith"错得自信"的物理形态；现有 gg 散落多处但无统一命名 |
| 2. L0 物理约束 | `@PROMPT_PREMATURE` | **被 G5 第 1 条吸收** | 工具返回前禁止完成态语言 |
| 2. L0 物理约束 | `@TRACE_OR_DIE` | **被 G5 第 2 条吸收** | 系统状态变更必须附物理证据 |
