...
1. **独立 Evaluator persona**：新增 `personas/evaluator.md`，在重大决策后装配 evaluator 对 reflection 做二次审查。代价：persona 膨胀
...
4-18 把 Generator-Evaluator 登记为 STRATEGIC Tier 3「9 天没解」，今日（4-27）以 5 小时 6 轮修复付账——Keith 被迫当 Evaluator 四次校准才推进。根因仍是"reflection 模板字段引力方向错误"（见 essence `field-gravity-over-prompt` / `reverse-anchor-by-reflection`）。修复落地在 `.template.md` 按 status 分流为范式 A/B + mirror 字段，绕过 thinking → final message 不可靠通道。v2+ 议题：inline 装配 / subagent 切分。
