...
> ⚠️ **过滤器盲区警告（2026-04-14 stable-identifiers 实际执行教训）**：用 `grep` 做"排除某种形式"的过滤时，如果"被排除形式"和"目标匹配"可能共存一行（典型：`**P0**` 和 `[P0]` 同时出现在一行），**禁止**用行级 `grep -v` —— 它会掩盖整行，漏掉真正违规的匹配。必须用 `grep -oE` 把每个匹配切出来单独处理，或用 perl/awk 做匹配级后过滤。本节下方所有 grep 命令在跨文件扫描时都遵循这个原则。
