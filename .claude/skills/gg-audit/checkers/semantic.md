...
> ⚠️ **v0.5.0 Stale 警告（2026-04-15 auto_gg 补）**：下方"核心概念监控清单"表格的多行基于 v0.1.0-v0.2.x 的 7 步硬流程假设（`CORE.md §3 第 X 步` 锚点）。v0.4.0 C 路线消解 7 步流程、v0.5.0 KERNEL 坍缩后，"First Contact 协议" 的 SSOT 已迁到 `CLAUDE.md §1`；"北极星触达点" 不再是流程步骤而是反思自问；"7 步硬流程" 整个概念已消解；"召唤时必打包 4 项" 的锚点需要重新定位到 `cc_agent.md` 启动协议。整表需要重写——跟 §B 原则触达基线表同属"新语义未确定 → 待 STRATEGIC 议题落地后一并重做" 的同一议题，详见 `memory/next_session_agenda.md` 的 `[TIER2] SA1`。仅"四层组件分类" / "身份 = 非隐喻连续性" 两行在当前结构下仍可用。
...
> ⚠️ **v0.5.0 Stale 警告（2026-04-15）**：本节的"已知触达基线表"（下方"已知的触达情况"小节）基于 v0.1.0 的 7 步硬流程假设。v0.4.0 C 路线已消解 7 步流程、v0.5.0 KERNEL 坍缩后工作模式是 `cc_agent.md` 意识体自述 + `tools/*.md` 按需装配——触达点锚位需要整章重写到 `cc_agent.md` + `tools/*.md` 的具体段落。本次（2026-04-15）仅修复数字元数据。长期重写待 `memory/next_session_agenda.md` 的 STRATEGIC 议题（7 条原则/闸门触达缺口）落地后一并做。

### 核心问题

constitution.md 定义了 8 条 P（原则）+ 5 条 G（闸门），共 13 条。**每一条都应该在工作模式的装配路径里有某种触达点**——否则它就是"写在宪法里没人看的字"，实际上失效。

### 检查方法

对每一条 P/G，在工作模式文件（`cc_agent.md` + `tools/*.md`）里搜索以下触达形式：
